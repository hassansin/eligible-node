...
function handleResponse(response, body) {
  if (response.statusCode === 401) {
    return new Promise.reject(new errors.AuthenticationError('Authentication \
      or authorization error. Mostly due to invalid api_key or provider need \
      enrollment with payer.', response.statusCode));
  } else if (response.statusCode === 404 || response.statusCode === 400) {
    return new Promise.reject(
      new errors.InvalidRequestError('Not Found',response.statusCode)
    );
  } else if (response.statusCode === 400) {
    return new Promise.reject(new errors.InvalidRequestError('Bad request, \
      invalid or missing parameters.', response.statusCode));
  ...
  try {
    ...
    return new Promise.reject(
      new errors.APIErrorResponseError(e.message, 0, body)
    );
  }
}
