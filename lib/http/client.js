...
function handleResponse(response, body) {
  ...
  if (response.statusCode === 401) {
    error = new Promise.reject(
      ...
      ' need enrollment with payer.', response.statusCode)
    );
    ...
    error = new Promise.reject(new errors.InvalidRequestError('Bad request,' +
      ' invalid or missing parameters.', response.statusCode));
  } else if (response.statusCode !== 200) {
    error = new Promise.reject(new errors.APIError(body, response.statusCode));
  }
  if (error) {
    return error;
  }
